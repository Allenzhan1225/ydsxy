...
- (void)openDB{
    ...
    NSString *path=[documentPath stringByAppendingString:@"/CollectionList.sqlite"];
    // 打印地址
//    NSLog(@"%@",documentPath);
    ...
    if (result==SQLITE_OK) {
        NSLog(@"数据库打开成功");
        //准备sql语句
        NSString *sql=@"CREATE TABLE CollectionList (ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name TEXT NOT NULL, c_type_1 INTEGER NOT NULL, docent_name TEXT NOT NULL, s_date TEXT NOT NULL, n_date TEXT NOT NULL, pic TEXT NOT NULL, status INTEGER NOT NULL, obj_id TEXT NOT NULL, introduce TEXT NOT NULL);";
        // 执行sql语句
...
- (void)insertCourses:(CoursesModel *)course{
    ...
    NSString *sql=@"INSERT INTO  CollectionList (ID,name,c_type_1,docent_name,s_date,n_date,pic,status,obj_id,introduce) VALUES (?,?,?,?,?,?,?,?,?,?)"; // ?的作用是占位，因为需要添加的数据时不固定的。
    // 4.验证sql是否正确
...
- (void)deleteCoursesWithCourseID:(int)courseID{
    ...
    sqlite3_stmt *stmt=nil;
    // 3.准备sql语句
    NSString *sql=(courseID!=0)?@"DELETE FROM CollectionList WHERE ID=?":@"DELETE FROM CollectionList";
    // 4.验证语句是否正确
...
- (NSArray *)selectAllCourses{
    ...
    sqlite3_stmt *stmt=nil;
    // 3.准备
    NSString *sql=@"SELECT *FROM CollectionList";
    // 4.验证
    int result=sqlite3_prepare_v2(db, [sql UTF8String], -1, &stmt, NULL);
    if (result==SQLITE_OK) {
//        NSLog(@"查询成功");
...
- (CoursesModel *)selectCoursesCoursesID:(int)coursesID{
    [self openDB];
    sqlite3_stmt *stmt=nil;
    NSString *sql=@"SELECT *FROM CollectionList WHERE ID=?";
    // 验证
    int result=sqlite3_prepare_v2(db, [sql UTF8String], -1, &stmt, NULL);
    if (result==SQLITE_OK) {
//        NSLog(@"查询成功");
